...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        {
            for (int number = 1; number <= 9999999; number++)
            {
            ...
            throw new InvalidOperationException("All UIDs have been exhausted");
        }
        
        private bool UIDExists(string uid)
        {
            return db.Students.Any(s => s.UId.ToLower() == uid.ToLower()) ||
                   db.Professors.Any(p => p.UId.ToLower() == uid.ToLower()) ||
                   db.Administrators.Any(a => a.UId.ToLower() == uid.ToLower());
        }
